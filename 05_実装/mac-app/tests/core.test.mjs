...
import {
  ...
  extractClientSecret,
  groupAudioSourceApps,
  INPUT_LANGUAGES,
  isAllowedLanguage,
  modeHasAudio,
  modeHasCaptions,
  OUTPUT_LANGUAGES,
  parseMacApplicationProcesses,
...
test("transcript sentences become scrollable display lines", () => {
  assert.deepEqual(
    splitTranscriptSentences("こんにちは。今日はいい天気ですね。"),
    ["こんにちは。", "今日はいい天気ですね。"],
  );
  assert.equal(
    captionDisplayText("Hello. It's fine today."),
    "Hello.\nIt's fine today.",
  );
});
...
test("transcript export supports individual, interleaved, and grouped layouts", () => {
  const input = {
    language1Text: "こんにちは。今日はいい天気ですね。",
    language2Text: "Hello. It's fine today.",
  };
  assert.equal(
    buildTranscriptExport({ ...input, scope: "language1" }),
    "こんにちは。\n今日はいい天気ですね。",
  );
  assert.equal(
    buildTranscriptExport({ ...input, scope: "both", layout: "interleaved" }),
    "こんにちは。\nHello.\n今日はいい天気ですね。\nIt's fine today.",
  );
  assert.equal(
    buildTranscriptExport({ ...input, scope: "both", layout: "grouped" }),
...
test("client secret supports current response shapes", () => {
  ...
  assert.equal(
    extractClientSecret({ client_secret: { value: "secret-b" } }),
    "secret-b",
  );
});
...
test("official language lists are exposed", () => {
  ...
  assert.deepEqual(
    Object.fromEntries(
      OUTPUT_LANGUAGES.map(({ code, nativeLabel }) => [code, nativeLabel]),
    ),
    {
      es: "Español",
      pt: "Português",
...
test("source-only captions keep one translation transport session", () => {
  const targets = buildTranslationTargets({
    mode: "captions",
    inputLanguage: "zh",
    outputLanguage: "zh",
    captionLanguages: ["source", "none"],
  });
...
test("macOS process output becomes a deduplicated app source list", () => {
  ...
  assert.deepEqual(
    parseMacApplicationProcesses(output).map(({ name }) => name),
    ["Google Chrome", "Music", "QuickTime Player"],
  );
});
...
test("common audio apps are separated from utility apps", () => {
  ...
  assert.deepEqual(
    groups.common.map(({ name }) => name),
    ["Music", "Google Chrome", "zoom.us", "Zoom Workplace"],
  );
  assert.deepEqual(
    groups.other.map(({ name }) => name),
    ["Logi Options+", "Mouse Driver Helper"],
  );
});

test("selected app audio is isolated from direct hardware output", async () => {
  const source = await readFile(
    new URL("../native/process_audio_tap.m", import.meta.url),
    "utf8",
  );
...
test("either caption slot can be disabled for a single translated subtitle", async () => {
  ...
  assert.match(
    renderer,
    /captionLanguage1\.replaceChildren\(\.\.\.captionLanguageOptions\(\)\)/,
  );
  assert.match(
    renderer,
    /captionLanguage2\.replaceChildren\(\.\.\.captionLanguageOptions\(\)\)/,
  );
...
test("audio and caption modes can be switched while translation stays active", async () => {
  ...
  const lockBody =
    renderer.match(
      /function lockLanguageControls\(locked\) \{([\s\S]*?)\n\}/,
    )?.[1] || "";
  ...
  assert.match(
    renderer,
    /closeTranslation\(\);[\s\S]*await startTranslation\(\)/,
  );
});
...
test("input, output, and both caption languages can change during translation", async () => {
  ...
  for (const control of [
    ...
    assert.match(
      renderer,
      new RegExp(`elements\\.${control}\\.addEventListener\\("change"`),
    );
  }
});
...
test("a key changed in settings overrides an environment fallback", async () => {
  ...
  const storedFunction =
    main.match(/async function storedApiKey\(\) \{([\s\S]*?)\n\}/)?.[1] || "";
  assert.ok(
    storedFunction.indexOf("settings.encryptedApiKey") <
      storedFunction.indexOf("process.env.OPENAI_API_KEY"),
  );
});
...
test("the API key is cached and migrated away from repeated Keychain access", async () => {
  ...
  const storedFunction =
    main.match(/async function storedApiKey\(\) \{([\s\S]*?)\n\}/)?.[1] || "";
  assert.ok(
    storedFunction.indexOf("apiKeyCache") <
      storedFunction.indexOf("safeStorage.decryptString"),
  );
...
test("app name and Sonic Glass icon are packaged consistently", async () => {
  const [main, packageSource, packager, icon] = await Promise.all([
    readFile(new URL("../electron/main.mjs", import.meta.url), "utf8"),
    readFile(new URL("../package.json", import.meta.url), "utf8"),
    readFile(new URL("../scripts/package-mac.mjs", import.meta.url), "utf8"),
    readFile(new URL("../assets/live-interpreter.icns", import.meta.url)),
  ]);
  ...
  assert.match(packager, /assets", "live-interpreter\.icns"/);
  assert.equal(icon.subarray(0, 4).toString("ascii"), "icns");
  assert.match(
    main,
    /app\.getPath\("appData"\), "asaph-live-interpreter-mac", "settings\.json"/,
  );
  const index = await readFile(
    new URL("../renderer/index.html", import.meta.url),
    "utf8",
  );
  assert.match(index, /class="sonic-logo"/);
  assert.doesNotMatch(
    index,
    /多言語ライブ通訳|Mac版|会議音声・動画・配信に対応|Live Interpreter Mixer|asaph-live/i,
  );
});
...
test("settings offers every realtime output language as an app language", async () => {
  ...
  for (const { code } of OUTPUT_LANGUAGES)
    assert.equal(validUiTranslations(builtinUiTranslations(code)), true, code);
  assert.match(main, /options: OUTPUT_LANGUAGES/);
  assert.match(main, /builtinUiTranslations\(code\)/);
  assert.doesNotMatch(
    main,
    /createUiTranslations|asaph-live-interpreter-ui-language|Target language:/,
  );
...
test("external caption chrome only appears while the pointer is over the window", async () => {
  ...
  assert.match(
    css,
    /body\.transparent\s*\{[^}]*background:\s*rgba\(5, 7, 11, \.46\)/s,
  );
  ...
  assert.match(
    main,
    /setOpacity\(latestCaptionPayload\.transparent \? 0\.72 : 1\)/,
  );
});
