...
$payments=$pdo->query('SELECT p.id,u.email,pr.code,p.amount_minor,p.refunded_minor,p.currency,p.status,p.created_at FROM payments p JOIN users u ON u.id=p.user_id JOIN products pr ON pr.id=p.product_id ORDER BY p.created_at DESC LIMIT 20')->fetchAll();
