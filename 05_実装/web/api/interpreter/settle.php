...
try {
    ...
    $stmt = $pdo->prepare('SELECT s.user_id,s.status,w.trial_seconds,w.paid_seconds FROM translation_sessions s JOIN wallets w ON w.user_id=s.user_id WHERE s.id=? FOR UPDATE');
    ...
    $remainingRequest = $seconds;
    $trialUsed = min($remainingRequest, (int)$session['trial_seconds']);
    $remainingRequest -= $trialUsed;
    $paidUsed = min($remainingRequest, (int)$session['paid_seconds']);
    $consumed = $trialUsed + $paidUsed;
    if ($consumed > 0) {
        $pdo->prepare('UPDATE wallets SET trial_seconds=trial_seconds-?,paid_seconds=paid_seconds-?,version=version+1 WHERE user_id=?')
            ->execute([$trialUsed,$paidUsed,$session['user_id']]);
