...
try {
    ...
    $stmt = $pdo->prepare('SELECT t.id token_id,t.user_id,t.new_email,u.trial_granted_at FROM email_verification_tokens t JOIN users u ON u.id=t.user_id WHERE t.token_hash=? AND t.used_at IS NULL AND t.expires_at>NOW() FOR UPDATE');
    ...
    if($record['new_email']){$pdo->prepare('UPDATE users SET email=?,email_verified_at=NOW() WHERE id=?')->execute([$record['new_email'],$record['user_id']]);}
    else{$pdo->prepare("UPDATE users SET status='active', email_verified_at=COALESCE(email_verified_at,NOW()) WHERE id=?")->execute([$record['user_id']]);}
    $trialSeconds=0;
    if (!$record['new_email']&&!$record['trial_granted_at']) {
        ...
        if((int)($claims['ip_claims']??0)<3&&(int)($claims['device_claims']??0)<1){
            ...
            $grant = $pdo->prepare("INSERT IGNORE INTO credit_ledger (id,user_id,entry_type,trial_delta,idempotency_key,note) VALUES (?,?,'trial_grant',720,?,'Initial 720-credit trial')");
            $grant->execute([uuid_v4(), $record['user_id'], $key]);
            if ($grant->rowCount() === 1 && grant_credit_lot($pdo,$record['user_id'],'trial','trial',$record['user_id'],720,30)) $trialSeconds=720;
